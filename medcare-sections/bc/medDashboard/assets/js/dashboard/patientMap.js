   ...
   var $button,
      ...
      initEvents = function () {
         ...
         google.maps.event.addListener(marker, "dragend", function () {
            ...
            //    console.log("got points", results[0]);
               if (results[0]) {
                  bindDataToForm(
          ...
          function bindDataToForm(address, lat, lng) {
            //  console.log("binding function is here");
            document.getElementById("patient-streetNo").value = address;
         }
      ...
      updatePacButtonState = function (loading) {
         if (loading) {
            ...
            $icon.removeClass("fa-location-arrow").addClass("fa-spinner fa-spin").prop("disabled", true);
         ...
         if ("" !== $pac.val()) {
            ...
            $icon.removeClass("fa-location-arrow fa-spinner fa-spin").addClass("fa-times");
         } else {
            $button.attr("data-original-title", $button.attr("data-title-geolocate"));
             $icon.removeClass("fa-times fa-spinner fa-spin").addClass("fa-location-arrow");
             document.getElementById("patient-streetNo").value = "";
         }
      },
      showMap = function () {
         if (mapIsVisible) {
            ...
            function (pos) {
               ...
               geocoder.geocode(
                  ...
                  function (results, status) {
                      if (status === google.maps.GeocoderStatus.OK) {
                        //   console.log("results area",results[0]);
                        if (results[0]) {
